...
func (a *App) newPassHash(password string, salt []byte) (p PassHash) {
	const (
		argonTimes   = 1
		argonMem     = 64 * 1024 // MB.
		argonThreads = 4
	...
	buf := make([]byte, len(a.cfg.Secret), len(a.cfg.Secret)+len(pass))
	copy(buf, a.cfg.Secret)
	p.Hash = argon2.IDKey(append(buf, pass...), p.Salt, argonTimes, argonMem, argonThreads, argonOut)
	return p
}

func (a *App) equalPassHash(password string, p PassHash) bool {
